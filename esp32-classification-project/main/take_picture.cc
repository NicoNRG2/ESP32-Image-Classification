...
esp_err_t init_camera(void)
{
    ...
    sensor_t *s = esp_camera_sensor_get();      // OV3660_PID
    s->set_vflip(s, 1);
...
esp_err_t grayscale_image(uint8_t *image_array) {
    ...
    // Copy image data to the provided array
    memcpy(image_array, pic->buf, pic->len);

    ESP_LOGI(TAG, "Picture taken and saved to array! Its size was: %zu bytes", pic->len);
